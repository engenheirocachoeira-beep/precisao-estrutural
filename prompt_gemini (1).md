...
- **Item 17, melhorar a proteção de colisão pra além do simples
  aviso** (fusão automática? bloqueio do rename?) — ainda sem decisão
  do usuário, fica em aberto pra quando/se acontecer de novo.

**Teste real no navegador, primeira leva já feita**: usuário conferiu
os projetos depois da correção do item 17 — **confirmado, projetos B
e C ainda estão sem Etapas na Árvore** (esperado — são exatamente os
casos órfãos "OBRA B"/"OBRA C" identificados durante a investigação;
a correção desta rodada impede o problema de acontecer de NOVO, mas
não recupera retroativamente dados que já ficaram órfãos antes dela).
**Todos os demais projetos testados, OK.** Usuário decidiu NÃO
recuperar os dados órfãos de B e C (nem manualmente via Console, nem
esperar uma ferramenta de recuperação) — vai **relançar (recriar) os
dois projetos do zero** e ver como fica. Pendência de recuperação em
massa fica descartada por decisão do usuário, não é mais uma tarefa
em aberto.
...
correção crítica do item 17); (2) "OBRA A" e "OBRA C" não vão mais ser
recuperadas — usuário decidiu recriar os projetos B e C do zero em vez
de recuperar dado órfão (ver nota logo acima); (3) itens 8, 14, e o
resto do item 17, quando for a vez deles.

**Status no fim desta conversa: sistema disponibilizado pra TESTE com
a equipe** (não é lançamento em produção ainda) — usuário subiu a
versão atualizada no link do Netlify
(https://gleaming-scone-2511ce.netlify.app) especificamente pra a
equipe testar em grupo pequeno antes de virar uso real. Avisos
passados pro usuário repassar à equipe: sempre acessar pelo link
(nunca `file://`), evitar editar o MESMO projeto em abas/dispositivos
diferentes ao mesmo tempo (risco de sincronização "último que salva
vence", ainda em aberto), e como reportar problemas (nome do projeto,
o que fez antes de notar, print do Console se der erro). **Aguardando
retorno da equipe** — não há próxima ação definida até os comentários
chegarem; quando chegarem, essa é a prioridade de abertura da próxima
conversa.
